...
public class ConversationEditorWindow : EditorWindow
{
	...
	[MenuItem ("Adventure Creator/Editors/Conversation Editor")]
	static void Init ()
	{
	...
	private void OptionsGUI ()
	{
		...
		scrollPosition = GUI.BeginScrollView (new Rect (0, 0, position.width, position.height), scrollPosition, new Rect (0, 0, 1100, 70 * (conversation.options.Count + 2)), false, false);
		BeginWindows ();
		...
		for (int i=0; i<conversation.options.Count; i++)
		{
			...
			if (conversation.options[i].dialogueOption != null)
			{
				if (GUI.Button (interactionRect, ""))
				{
					if (conversation.interactionSource == ActionListSource.InScene)
					{
	...
	private void NodeWindow (int ID)
	{
		...
		else if (ID < conversation.options.Count * 2)
		{
			...
			if (conversation.interactionSource == ActionListSource.AssetFile)
			{
			...
			else
			{
				if (conversation.options[i].dialogueOption != null)
				{
					GUILayout.Label (conversation.options[i].dialogueOption.gameObject.name);
				}
				else
				{
					GUILayout.BeginHorizontal ();
		...
		else
		{
			...
			else
			{
				...
				if (GUILayout.Button ("Auto-create"))
				{
					...
					Conversation newConversation = AdvGame.GetReferences ().sceneManager.AddPrefab ("Logic", "Conversation", true, false, true).GetComponent <Conversation>();
