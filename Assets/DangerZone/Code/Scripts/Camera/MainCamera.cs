...
namespace AC
{
	...
	public class MainCamera : MonoBehaviour
	{
		...
		public void SetFirstPerson ()
		{
			if (settingsManager)
			{
				if (settingsManager.IsInFirstPerson ())
				{
		...
		private void SetFadeTexture (Texture2D tex)
		{
			...
			{
				GameObject.FindWithTag (Tags.persistentEngine).GetComponent <StateHandler>().tempFadeTexture = tex;
			}
			AssignFadeTexture ();
		}
		...
		public void ReleaseFadeTexture ()
		{
			if (GameObject.FindWithTag (Tags.persistentEngine))
			{
				GameObject.FindWithTag (Tags.persistentEngine).GetComponent <StateHandler>().tempFadeTexture = null;
			}
			AssignFadeTexture ();
		}
		...
		{
			if (GameObject.FindWithTag (Tags.persistentEngine) && GameObject.FindWithTag (Tags.persistentEngine).GetComponent <StateHandler>().tempFadeTexture != null)
			{
				actualFadeTexture = GameObject.FindWithTag (Tags.persistentEngine).GetComponent <StateHandler>().tempFadeTexture;
			}
			else
			{
		...
		private bool SetAspectRatio ()
		{
			...
			if (!settingsManager.forceAspectRatio || (int) (currentAspectRatio * 100) / 100f == (int) (settingsManager.wantedAspectRatio * 100) / 100f)
			{
				borderWidth = 0f;
