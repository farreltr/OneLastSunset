...
namespace AC
{
	...
	public class ActionListEditor : Editor
	{
		...
		public static int GetTypeNumber (AC.Action action)
		{
			int number = 0;
			...
			if (actionsManager)
			{
				for (int j=0; j<actionsManager.GetActionsSize(); j++)
				{
					try
					{
						if (action.GetType ().ToString () == actionsManager.GetActionName (j) || action.GetType ().ToString () == ("AC." + actionsManager.GetActionName (j)))
						{
							number = j;
							break;
						}
					...
					catch
					{
						...
						action = (AC.Action) CreateInstance (defaultAction);
					}
				}
			}
		...
		public static int GetSubCategoryNumber (string actionTitle, int _categoryNumber)
		{
			int number = 0;
			...
			if (actionsManager)
			{
				return actionsManager.GetActionSubCategory (actionTitle, _categoryNumber);
			}
		...
		public static AC.Action RebuildAction (AC.Action action, int number, ResultAction _resultAction, int _skipAction, ActionListAsset _linkedAsset, Cutscene _linkedCutscene)
		{
			ActionsManager actionsManager = AdvGame.GetReferences ().actionsManager;
