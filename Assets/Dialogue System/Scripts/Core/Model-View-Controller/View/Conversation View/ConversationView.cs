...
namespace PixelCrushers.DialogueSystem
{
	...
	public class ConversationView : MonoBehaviour
	{
		...
		/// sequence).
		/// </summary>
		public EventHandler FinishedSubtitleHandler = null;
		
		/// <summary>
		/// Called when the player selects a response.
		/// </summary>
		public EventHandler<SelectedResponseEventArgs> SelectedResponseHandler = null;
		...
		public void StartSubtitle (Subtitle subtitle, bool isPCResponseMenuNext, bool isPCAutoResponseNext)
		{
			if (subtitle != null) {
				if (DialogueDebug.LogInfo)
					Debug.Log (string.Format ("{0}: {1} says '{2}'", new System.Object[] {
		...
		{
			UnityUITypewriterEffect subtitle = GameObject.FindObjectOfType<UnityUITypewriterEffect> ();
			if (subtitle != null && subtitle.IsPlaying) {
				subtitle.Stop ();
			} else {
		...
		private void FinishSubtitle ()
		{
			if (!waitForContinue) {
				if (sequencer != null)
					sequencer.Stop ();
				if (lastNPCSubtitle != null)
					ui.HideSubtitle (lastNPCSubtitle);
				if (lastPCSubtitle != null)
					ui.HideSubtitle (lastPCSubtitle);
		...
		private void NotifyParticipantsOnConversationLine (Subtitle subtitle)
		{
			if (subtitle != null) {
				...
				if (validSpeakerTransform)
					subtitle.speakerInfo.transform.BroadcastMessage ("OnConversationLine", subtitle, SendMessageOptions.DontRequireReceiver);
				if (validListenerTransform && !speakerIsListener)
					subtitle.listenerInfo.transform.BroadcastMessage ("OnConversationLine", subtitle, SendMessageOptions.DontRequireReceiver);
