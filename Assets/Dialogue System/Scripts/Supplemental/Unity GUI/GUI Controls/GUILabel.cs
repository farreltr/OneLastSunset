	...
	public class GUILabel : GUIVisibleControl
	{
		...
		public override void DrawSelf (Vector2 relativeMousePosition)
		{
			...
			if (!string.IsNullOrEmpty (text)) {
				if (useSubstring || this.isTypeWriter) {
					DrawSubstring ();
				} else {
		...
		private void DrawSubstringLeftAligned ()
		{
			...
			while (charsLeft > 0) {
				...
				string subLine = line.Substring (0, Mathf.Min (line.Length, charsLeft)).Trim ();
				start += line.Length;
				charsLeft -= line.Length;
				UnityGUITools.DrawText (new Rect (rect.x, y, rect.width, lineHeight), GetRichTextClosedText (subLine.Trim ()), GuiStyle, textStyle, textStyleColor);
				y += lineHeight;
			}
				...
				while (start + length < text.Length) {
					...
					if (width < rect.width) {
						...
						if (lastSpace > 0) {
							...
							string substr = text.Substring (start, Mathf.Max (1, Mathf.Min (lastSpace, maxLength)));
							return substr;
						} else {
							string substr = text.Substring (start, Mathf.Max (1, Mathf.Min (length - 1, maxLength)));
							return substr;
						}
					}
				}
